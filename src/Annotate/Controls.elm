module Annotate.Controls exposing (Config, Dropdown, DropdownTrigger(..), DropdownType(..), Msg(..), State, closeDropdown, initialState, onKeyDown, subscriptions, update, view)

import Annotate.Environment exposing (OperatingSystem(..))
...
import Time
import Utils exposing (isSpotlight)
...
button config annotation =
    Html.button
        [ classList
            [ ( "annotation-button", True )
            , ( "annotation-button--selected", config.selected == annotation )
            , ( "annotation-button--spotlight", isSpotlight annotation )
            ]
        , title (config.toTitle annotation)

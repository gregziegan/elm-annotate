...
@docs hasNoText, resizeFn, startAndEnd
...
{-| Tweak an annotation's presentation
-}
type alias Styles =
    { strokeColor : StrokeColor
    , fill : Fill
...
{-| Return where the user started and finished drawing
-}
...
{-| Move an annotation by some x and y
-}
...
{-| Presents a potentially interactive annotation
-}
...
{-| The SVG definition for an annotation.
-}
type Def msg
...
{-| Presents a graphic to be placed within an SVG <defs /> tag
-}
viewDefinition : Config msg -> Annotation -> Def msg
viewDefinition { events, snap } { start, end, styles, choice } =
    let
        shape =
...
viewTextBox { onInput } { id, start, end, autoExpand, text, styles } =
    foreignObject
        [ Attr.y <| inPixels <| -10 + Basics.min start.y end.y
        , Attr.x <| inPixels <| -20 + Basics.min start.x end.x
        , Attr.width "100%"
        , Attr.height "100%"
        ]
        [ div
            [ class "text-box-container"
            , style "font-size" (inPixels styles.fontSize)
